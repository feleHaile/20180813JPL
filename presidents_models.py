...
from EXAMPLES.presidents_one.models import President
...
def president_by_term(termnum):
    ...
    term = int(termnum)
    if 0 < term < 45:
        p = President.query(termnum=termnum)
...
def president_by_last_name(last_name):
    """Retrieve president information for all or part of a last name;
    ...
    for i in range(1, 45):
        p = President(i)
        if last_name.lower() in p.last_name.lower():
            presidents.append(p)
